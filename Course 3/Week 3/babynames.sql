...
SELECT name,count
FROM
    `data-robi.babynames.names_2014`
WHERE gender="M"
ORDER BY count desc
LIMIT 5;
...
FROM
    ...
    FROM
        `data-robi.babynames.names_2015`)
    UNION ALL
    (SELECT name,count, 2016 as tahun
    FROM
        `data-robi.babynames.names_2016`)
    UNION ALL
    (SELECT name,count, 2017 as tahun
    FROM
        `data-robi.babynames.names_2017`)
    UNION ALL
    (SELECT name,count, 2018 as tahun
    FROM
        `data-robi.babynames.names_2018`)
    UNION ALL
    (SELECT name,count, 2019 as tahun
    FROM
        `data-robi.babynames.names_2019`))
GROUP BY name
ORDER BY count desc
LIMIT 10;
